...
class Particle:
    ...
    def __repr__(self):
        args = [(attr, getattr(self, attr)) for attr in ["pos", "vel", "typ"] \
                if hasattr(self, attr)]
        return "Particle(%s)" % ", ".join("%s=%r" % a for a in args)
...
def lookup_table(system, L=10):
    ...
    for i in range(N):
        for j in range(i):
            r_d = norm(system[i].pos - system[j].pos)
            r_x = norm(border_dist(system[i].pos, system[j].pos, 0, L))
            r_y = norm(border_dist(system[i].pos, system[j].pos, 1, L))
            r_z = norm(border_dist(system[i].pos, system[j].pos, 2, L))
            #table.append((i, j, r_d, r_x, r_y, r_z))  # testing
            if r_d < 1:
               table.append((i, j, -1, r_d))
            elif r_x < 1:
