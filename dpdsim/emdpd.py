...
import time
import pandas as pd
from .Fdpd.emdpd_f import emdpd_f
...
class EMDPDSim():
    ...
    def compute_pe(self):
        if self.imp == "numba":
            ...
            return emdpd_f.compute_pe(self.X, self.rho2, self.bl, \
    ...
    def compute_local_density(self):
        if self.imp == "numba":
            ...
            self.rho2 = emdpd_f.local_density(self.X, self.bl, self.Nbt, \
    ...
    def compute_force(self):
        ...
        if self.imp == "numba":
            ...
            emdpd_f.compute_force(self.F, self.vir, self.sigma, \
    ...
    def compute_force_cube(self):
        if self.imp == "numba":
            ...
            self.Fcube = emdpd_f.compute_force_cube(\
                self.X, self.V, self.rho2, self.bl, \
                self.ip_A[1:, 1:], self.ip_B, self.Rd[1:, 1:], \
                self.box, self.gama, self.kT, self.dt)
...
def force_numba(X, V, rho2, bl, ip_A, ip_B, Rd, box, gamma, kT, dt):
    ...
    for i in range(N):
        for j in range(i):
            ...
            rhoj = rho2[j, bl[i]]
            rd = Rd[bl[i], bl[j]]
            nrm = 15.0 / (2.0*pi*rd**3)

            fpair = A * wr(nr) \
                + ip_B * (rhoi + rhoj) * nrm * wr(nr/rd) * 2.0/rd \
